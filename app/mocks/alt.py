...
router = APIRouter(prefix="/mock/alt", tags=["mock_alt"])
...
def create_label(payload: dict):
    tracking = f"ALT-TRACK-{random.randint(100000, 999999)}"
    label_url = f"https://labels.alt.mock/{tracking}.pdf"
...
def push_event(payload: dict):
    ...
    orchestrator_url = payload.get("orchestrator_url") or "http://127.0.0.1:8000/webhooks/tracking/ALT"
